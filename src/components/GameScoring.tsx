...
const GameScoring: React.FC<GameScoringProps> = ({
  ...
  const handleEndGame = () => {
    ...
    if (!gameWinner && gameId) {
      ...
      saveGameToSupabase(gameId, playerData, actions, true, null);
    } else {
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-2xl max-w-lg w-full border-4 border-blue-500 dark:border-blue-600 dark:text-white">
            {gameWinner ? (
              <>
                <div className="text-center mb-3 animate-pulse">
                  <div className="text-4xl mb-1 flex justify-center items-center">
                    ...
                    <h3 className="text-xl font-bold text-blue-700 dark:text-blue-400 inline">
                      Game Completed!
                    </h3>
                ...
                <div className="mb-4">
                  <p className="mb-2 text-center">
                    ...
                    won with{' '}
                    <span className="font-bold">{gameWinner.score}</span>{' '}
                    points!
                  </p>
                      ...
                      <div className="grid grid-cols-2 gap-x-3 gap-y-0">
                        {playerData.map((player) => (
                          <div
                            key={player.id}
                            className={`text-sm ${
                              player.id === gameWinner.id ? 'font-semibold' : ''
                            }`}
                          >
                            <div>
                              {player.name}{' '}
                            ...
                            <div className="grid grid-cols-3 gap-x-1 text-xs mt-1">
                              ...
                              <div>
                                BPI:{' '}
                                {(
                                  player.score / Math.max(1, player.innings)
                                ).toFixed(2)}
                              </div>
                            </div>
                          </div>
              ...
              <>
                ...
                <p className="mb-6">
                  Are you sure you want to start a new game? The current game
                  will be saved and ended.
                </p>
              </>
            )}
              ...
              <div className="grid grid-cols-5 gap-2">
                {botAction === 'newrack'
                  ...
                  : // For other actions, create an array from 2 to current ballsOnTable
                    Array.from(
                      { length: Math.max(0, ballsOnTable - 1) },
                      (_, i) => i + 2
                    ).map((num) => (
                      <button
                        key={num}
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {(() => {
                      ...
                      let currentRun = 0;
                      let inningStartTime: Date | null =
                        actions.length > 0
                          ? new Date(actions[0].timestamp)
                          : null;
                      ...
                      actions.forEach((action, idx) => {
                        if (action.type === 'score') {
                          ...
                          ['miss', 'safety', 'foul'].includes(action.type)
                        ) {
                          ...
                          const nextPlayerId = playerData.find(
